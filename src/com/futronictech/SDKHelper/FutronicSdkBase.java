...
public abstract class FutronicSdkBase
{
    ...
    public static final int RETCODE_INCOMPATIBLE_SOFTWARE = (FTR_RETCODE_DEVICE_BASE + 9);
    ...
    public static String SdkRetCode2Message(int nRetCode)
    {
        ...
        case RETCODE_INCOMPATIBLE_SOFTWARE:
            szMessage = new String( "Incompatible software." );
            break;
                             ...
                             byte[] pBitmap )
    {
        ...
        if( (StateMask & FTR_STATE_FRAME_PROVIDED) != 0 )
        {
        	...
        	for( int i=0; i<BitmapWidth * BitmapHeight; i++)
        		pixels[i] = pBitmap[i];
        	Bitmap emptyBmp = Bitmap.createBitmap(pixels, BitmapWidth, BitmapHeight, Config.RGB_565);

            int width, height; 
            height = emptyBmp.getHeight(); 
            width = emptyBmp.getWidth();     
         
            Bitmap mBitmapFP = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565); 
            ...
            m_CallBack.UpdateScreenImage( mBitmapFP );
        }
    ...
    protected void SetIoCtx(Object ctx)
    {
    	m_IoCtx = ctx;
    }
    
    protected Object GetIoCtx()
    {
    	return m_IoCtx;
    }
    ...
    /**
    * <code>true</code> if the library should activate Fast Mode feature. You cannot modify this variable directly. Use the 
    ...
    */
    ...
    /*
     * Reference to UsbHostDataExchangeImplementation
     */
     ...
     */
    protected native int FutronicEnroll(Object io_ctx);
     ...
     * Function set parameters specific fro enrollment operation and does enrollment.
     */
    protected native int VerificationProcess(Object io_ctx);
     ...
     */
    protected native int GetBaseTemplateProcess(Object io_ctx);
     ...
     */
    protected native int IdentifyProcess( Object io_ctx, FtrIdentifyRecord[] rgTemplates, FtrIdentifyResult Result );
