...
public class UsbDeviceDataExchangeImpl
{
	...
	static final int transfer_buffer_size = 2 * 1024 * 64;
	...
	public boolean DataExchange(byte[] out_data, byte[] in_data, int in_time_out, int out_time_out, boolean keep_open, boolean use_max_end_point_size, int trace_level )
	{
		...
		try
		{
			...
			while( to_read_size >=  transfer_buffer_size_2 )
			{
				...
				if( transfer_bytes == -1 )
				{
					Log.e( log_tag , String.format( "Receive(2) %d bytes failed", getTransferBuffer().length ) );
					return res;
				}
			...
			if( to_read_size > usb_ctx.mReadPoint_.getMaxPacketSize() )
			{
				...
				if( data_left > 0 )
				{
					...
					if( transfer_bytes == -1 )
					{
						Log.e( log_tag , String.format( "Receive(3) %d bytes failed", data_left ) );
						return res;
					}
			...
			while( to_read_size > 0 )
			{
				...
				if( transfer_bytes == -1 )
				{
					Log.e( log_tag , String.format( "Receive(4) %d bytes failed", to_read_size ) );
					return res;
				}
